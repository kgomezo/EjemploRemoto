...
int main(int argc, char **argv) {
  ...
  int localsize = N/NTH;
  for (int i = 0; i < NTH; i++) {
    int th_min_global_idx = N*i/NTH; // minimum global index for thread i
    int th_max_global_idx =N*(i+1)/NTH ; // maximum global index for thread i. What happens if N%NTH != 0 ?
